...
const MOCK_BRACKET: BracketStage[] = [
  { 
    // 🚀 AŞAMA İSİMLERİ GLOBAL (İNGİLİZCE) YAPILDI. ÇEVİRİ BEYNİ BUNU TÜRKÇE YAPACAK!
    title: 'Quarterfinals', 
    matches: [ 
...
export default function TournamentDetail({ tournament, onBack }: { tournament: any, onBack: () => void }) {
  ...
  if (selectedMatch) {
    ...
    return (
      <div className="flex-1 flex flex-col h-full transition-colors" style={{ background: 'var(--es-bg)' }}>
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-hidden animate-fade-in transition-colors" style={{ background: 'var(--es-bg)' }}>
      <div className="shrink-0 relative overflow-hidden transition-colors" style={{ background: 'var(--es-bg-2)', borderBottom: '1px solid var(--es-border)' }}>
          ...
          <div className="flex items-end justify-between">
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded border transition-colors" style={{ background: 'var(--es-surface)', borderColor: 'var(--es-border)', color: 'var(--es-text-3)' }}><Shield className="w-3 h-3" style={{ color: gameColor }}/> {translateApiText(tournament.tier)}</span>
                <span className={`px-2.5 py-1 rounded border text-[10px] font-black uppercase tracking-widest ${tournament.status === 'live' ? 'bg-red-500/10 text-red-500 border-red-500/20' : tournament.status === 'completed' ? 'bg-slate-500/10 text-slate-500 border-slate-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'}`}>
                  {tournament.status === 'live' ? t.live : tournament.status === 'completed' ? t.completed : t.upcoming}
                </span>
              </div>
              <h1 className="text-4xl font-black tracking-tight transition-colors" style={{ color: 'var(--es-text-1)' }}>{tournament.name}</h1>
            </div>
            
            <div className="flex gap-6 p-4 rounded-xl border backdrop-blur-sm transition-colors" style={{ background: 'var(--es-surface)', borderColor: 'var(--es-border)' }}>
              <div className="flex flex-col gap-1"><span className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 transition-colors" style={{ color: 'var(--es-text-3)' }}><DollarSign className="w-3 h-3"/> {t.prizePool}</span><span className="text-base font-black text-green-500 transition-colors">{tournament.prizePool}</span></div>
              <div className="w-px transition-colors" style={{ background: 'var(--es-border)' }} />
              <div className="flex flex-col gap-1"><span className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 transition-colors" style={{ color: 'var(--es-text-3)' }}><Calendar className="w-3 h-3"/> {t.date}</span><span className="text-base font-black transition-colors" style={{ color: 'var(--es-text-1)' }}>{translateApiText(tournament.startDate)}</span></div>
            </div>
          </div>
        </div>

        <div className="px-8 border-t transition-colors" style={{ borderColor: 'var(--es-border)' }}>
          <div className="max-w-7xl mx-auto flex items-center gap-8">
            {[ { id: 'overview', label: t.overview, icon: Info }, { id: 'stats', label: t.statistics, icon: BarChart3 }, { id: 'teams', label: t.teams, icon: Users } ].map(tab => (
        ...
        <div className="flex justify-between min-w-[900px] h-[600px] pb-12">
          {bracket.map((stage, stageIndex) => (
            <div key={stageIndex} className="flex flex-col w-64 relative">
              <div className="text-[10px] font-black uppercase tracking-widest text-center mb-6 h-4" style={{ color: 'var(--es-text-3)' }}>
                 {/* 🚀 BURASI ARTIK İNGİLİZCE VERİYİ OTOMATİK ÇEVİRECEK */}
...
function BracketMatchCard({ match, stageIndex, isLastStage, onClick }: { match: BracketMatch, stageIndex: number, isLastStage: boolean, onClick: () => void }) {
  ...
  return (
    <div onClick={onClick} className="relative z-10 hover:z-[60] w-full rounded-xl overflow-visible shadow-xl border group cursor-pointer transition-colors" style={{ background: 'var(--es-surface)', borderColor: 'var(--es-border)' }}>
      
      {match.status === 'upcoming' && <div className="absolute inset-0 z-20 flex items-center justify-center backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: 'rgba(0,0,0,0.5)' }}><span className="text-xs font-black px-3 py-1 rounded-full transition-colors" style={{ background: 'var(--es-surface-2)', color: 'var(--es-text-1)' }}>{match.time}</span></div>}
      ...
      <div className="flex items-center justify-between p-3 border-b transition-colors" style={{ background: 'var(--es-bg)', borderColor: 'var(--es-border)' }}>
        <div className="flex items-center gap-2"><div className="w-4 h-4 rounded-sm" style={{ background: match.team1.color }} /><span className="text-xs font-bold transition-colors" style={{ color: 'var(--es-text-1)' }}>{match.team1.name}</span></div>
        <span className={`text-sm font-black transition-colors`} style={{ color: match.team1.score > match.team2.score ? 'var(--es-text-1)' : 'var(--es-text-3)' }}>{match.team1.score}</span>
      </div>
      <div className="flex items-center justify-between p-3 transition-colors" style={{ background: 'var(--es-bg)' }}>
        <div className="flex items-center gap-2"><div className="w-4 h-4 rounded-sm" style={{ background: match.team2.color }} /><span className="text-xs font-bold transition-colors" style={{ color: 'var(--es-text-1)' }}>{match.team2.name}</span></div>
        <span className={`text-sm font-black transition-colors`} style={{ color: match.team2.score > match.team1.score ? 'var(--es-text-1)' : 'var(--es-text-3)' }}>{match.team2.score}</span>
      </div>

      {(match.status === 'completed' || match.status === 'live') && match.maps && match.lineups && (
        <div className={`absolute ${tooltipPositionClass} top-1/2 -translate-y-1/2 w-80 border rounded-2xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-[100] p-5 pointer-events-none`} style={{ background: 'var(--es-bg-2)', borderColor: 'var(--es-border)', backdropFilter: 'blur(16px)' }}>
          
          <div className="flex justify-between items-center pb-3 border-b mb-3 relative transition-colors" style={{ borderColor: 'var(--es-border)' }}>
            {match.status === 'live' && (
          ...
          <div className="space-y-1.5 mb-5">
            {match.maps.map((m, i) => (
              <div key={i} className="flex justify-between items-center text-xs px-2 py-1 rounded transition-colors" style={{ background: 'var(--es-surface)' }}>
                ...
                <span className="font-bold transition-colors" style={{ color: 'var(--es-text-1)' }}>{m.name}</span>
          ...
          <div className="space-y-1">
            {match.lineups.map((l, i) => (
              <div key={i} className="flex justify-between items-center text-[11px]">
                <div className="w-1/3 text-left truncate font-semibold transition-colors" style={{ color: 'var(--es-text-1)' }}>{l.t1Player}</div>
                <div className="w-1/3 flex items-center justify-center gap-3"><span className="font-black" style={{ color: l.t1Rating > l.t2Rating ? 'var(--es-text-1)' : 'var(--es-text-3)' }}>{l.t1Rating.toFixed(2)}</span><span className="font-black" style={{ color: l.t2Rating > l.t1Rating ? 'var(--es-text-1)' : 'var(--es-text-3)' }}>{l.t2Rating.toFixed(2)}</span></div>
                <div className="w-1/3 text-right truncate font-semibold transition-colors" style={{ color: 'var(--es-text-1)' }}>{l.t2Player}</div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-xl p-6 shadow-lg flex flex-col items-center transition-colors" style={{ background: 'var(--es-card)', border: '1px solid var(--es-border)' }}>
         ...
         <div className="flex flex-wrap justify-center gap-4 mt-4">
           {distributionData.map((entry: any, index: number) => (
             <div key={index} className="flex items-center gap-2"><div className="w-3 h-3 rounded-sm" style={{ background: entry.color }} /><span className="text-xs font-bold transition-colors" style={{ color: 'var(--es-text-1)' }}>{entry.name} <span style={{ color: 'var(--es-text-3)' }}>(%{entry.value})</span></span></div>
           ))}
         </div>
      </div>
    </div>
