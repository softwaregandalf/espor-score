...
import {
  ...
  Bell, Search, ChevronDown, BarChart2, Radio, Gamepad2, X, Menu, History, Sun, Moon
} from "lucide-react";
  ...
  return (
    <div className="flex h-screen overflow-hidden transition-colors duration-500" style={{ background: 'var(--es-bg)' }}>
        ...
        <div className="p-4 border-t border-white/5">
          {!sidebarCollapsed ? (
            <button onClick={() => router.push('/profile')} className="flex items-center gap-3 w-full hover:bg-white/5 p-2 rounded-xl transition-colors">
              <div className="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-white">AD</div>
              <div className="flex flex-col text-left flex-1 min-w-0">
                <span className="text-sm font-bold text-white">{t.adminUser}</span>
                <span className="text-[10px] font-semibold text-slate-400">{t.proMember}</span>
              </div>
              <ChevronDown className="w-4 h-4 shrink-0 text-slate-500" />
            </button>
          ) : (
            <div onClick={() => router.push('/profile')} className="w-9 h-9 mx-auto rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-white cursor-pointer hover:bg-slate-700 transition-colors">AD</div>
          )}
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative z-10 transition-colors duration-500">
        ...
        <header className="h-[70px] flex items-center justify-between px-8 z-30 shrink-0 border-b border-white/5 bg-slate-900/50 backdrop-blur-md transition-colors">
          <div className="flex items-center gap-4 flex-1 max-w-xl">
            <div className="relative group w-full">
              <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 transition-colors" />
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t.searchPlaceholder} className="w-full py-2.5 pl-10 pr-4 rounded-xl text-sm outline-none transition-all bg-slate-800/50 border border-slate-700 text-white placeholder:text-slate-500" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {mounted && (
              <div className="relative group">
                <button className="flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all hover:scale-105" style={{ background: isDark ? 'rgba(255,255,255,0.05)' : '#ffffff', borderColor: isDark ? 'rgba(255,255,255,0.1)' : '#e2e8f0', boxShadow: isDark ? 'none' : '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                ...
                <div className="absolute right-0 top-full mt-2 w-32 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                  <button onClick={() => setLanguage('en')} className={`w-full flex items-center gap-3 px-4 py-2.5 text-xs font-bold transition-colors ${language === 'en' ? 'bg-es-cyan/10 text-es-cyan' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5'}`}>
                    <span className="text-base">🇬🇧</span> English
                  </button>
                  <button onClick={() => setLanguage('tr')} className={`w-full flex items-center gap-3 px-4 py-2.5 text-xs font-bold transition-colors ${language === 'tr' ? 'bg-es-cyan/10 text-es-cyan' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5'}`}>
            ...
            <button className="p-2 rounded-xl transition-colors hover:bg-black/5 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 relative">
